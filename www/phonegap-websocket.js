...
function hasWebSocket() {
  ...
  // WebRTC will be available either in Crosswalk or Web emulator
  // We assume if it is available, WebSocket will also be present
  // TODO: Check for PeerConnection instead?
  var hasWebRTC = navigator.getUserMedia || navigator.mozGetUserMedia || navigator.webkitGetUserMedia || window.getUserMedia;

  console.log('WebSocket hasConstructor: ' + hasConstructor + ', hasWebRTC: ' + (hasWebRTC ? true : false) );
  console.log('WebSocket m: ' + m);

  if (hasWebRTC) { return true }

  // This is not an Android Cordova
...
hasWebSocket() || (function() {
  ...
  console.log('WebSocket - building native');
